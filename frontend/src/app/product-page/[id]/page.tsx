...
const ProductPage: React.FC = () => {
  ...
  const { getItemById, bidOnItem, getUserNameByClerkId } = useApi();
  ...
  const [highestBidderName, setHighestBidderName] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const fetchItemDetails = async () => {
      try {
        ...
        if (fetchedItem.highestBid?.userId) {
          const userName = await getUserNameByClerkId(fetchedItem.highestBid.userId);
          setHighestBidderName(userName || "Anonymous");
        } else {
  ...
  const handleBid = async () => {
    ...
    const currentHighestBid =
      item?.highestBid?.amount || item?.startingPrice || 0;
    ...
    try {
      const bidResponse: BidResponse = await bidOnItem(id, {
        newBid: numericBid,
      });
              ...
              <div className="mt-6">
                <div className="flex space-x-2">
                  <input
                    ...
                    placeholder={`Minimum bid: ${formatPrice(
                      item.highestBid?.amount || item.startingPrice
                    )}`}
